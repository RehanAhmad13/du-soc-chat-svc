...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if self.thread.tenant_id != self.user.tenant_id:
            logger.warning(f"[WS] Tenant mismatch for user {self.username}")
            await self.close()
            return
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if msg_type == "message":
            ...
            await database_sync_to_async(ReadReceipt.objects.create)(
