...
from chat_svc.models import (
    ...
    MessageLog, User, ReadReceipt
)

from .serializers import (
    ChatThreadSerializer, MessageSerializer, QuestionTemplateSerializer,
    AttachmentSerializer, DeviceSerializer, UserSerializer, ReadReceiptSerializer
)

from .permissions import IsTenantMember, IsTenantOwner, IsActiveTenantMember
from integrations import event_bus, push, itsm
...
class ObtainJWTView(GenericAPIView):
    ...
    def post(self, request):
        ...
        if not user.tenant_id:
            return Response({"detail": "No tenant assigned"}, status=status.HTTP_403_FORBIDDEN)
...
class QuestionTemplateViewSet(viewsets.ReadOnlyModelViewSet):
    """Question templates available to tenant users"""
    queryset = QuestionTemplate.objects.all()
    serializer_class = QuestionTemplateSerializer
    permission_classes = [IsActiveTenantMember]

    def get_queryset(self):
        user = self.request.user
        # Return global templates and tenant-specific templates
...
class ChatThreadViewSet(viewsets.ModelViewSet):
    """Chat thread management for tenant users"""
    queryset = ChatThread.objects.all()
    serializer_class = ChatThreadSerializer
    permission_classes = [IsActiveTenantMember, IsTenantMember]

    def get_queryset(self):
        return self.queryset.filter(
            tenant_id=self.request.user.tenant_id
        ).select_related('tenant', 'template').prefetch_related(
            'messages__sender', 
            'messages__receipts__user',
            'messages__structured__template',
            'messages__attachments',
            'template_responses__user'
        ).order_by('-created_at')
    ...
    def perform_create(self, serializer):
        ...
        # Send push notifications to all tenant users
    ...
    @action(detail=True, methods=['post'], url_path='bulk-read')
    def bulk_read(self, request, pk=None):
        """Mark multiple messages as read"""
        thread = self.get_object()
        message_ids = request.data.get('message_ids', [])
        
        if not message_ids:
            return Response({"detail": "message_ids is required"}, status=400)
        
        # Filter messages that belong to this thread and aren't sent by current user
        ...
        # Return the newly created receipts
        serializer = ReadReceiptSerializer(created_receipts, many=True)
        return Response({
    ...
    def mark_all_read(self, request, pk=None):
        ...
        messages = thread.messages.exclude(sender=request.user)
        ...
        return Response({
            'marked_read_count': len(created_receipts),
            'total_messages': messages.count()
        })

    @action(detail=True, methods=['get'], url_path='read-receipts')
    def read_receipts(self, request, pk=None):
        """Get all read receipts for this thread"""
        thread = self.get_object()
        receipts = ReadReceipt.objects.filter(
            message__thread=thread
        ).select_related('message', 'user').order_by('-timestamp')
        
        serializer = ReadReceiptSerializer(receipts, many=True)
